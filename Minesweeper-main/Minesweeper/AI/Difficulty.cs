...
namespace Minesweeper.AI
{
    public class Difficulty
    {
        ...
        private void StartTest()
        {
            ...
            int solvableUnderLimit = Find3BVCountUnderLimit(values3BV, betchelsLimit);
            ...
            {
                for (int scale = 9; scale > 0; scale--)
                {
                    double percent = Math.Pow(10, magnitude) * scale;
                    System.Diagnostics.Debug.WriteLine("Luckiest " + percent + "%: " + FindPercentageCutoff(percent, values3BV));
                }
            }

            // outputs what the mean minus each multiple of the standard deviation is until it goes below 0
        ...
        private int Find3BVCountUnderLimit(List<int> values, int limit)
        {
            int topPointer = values.Count - 1;
            int rearPointer = 0;
            int midPointer = values.Count / 2;
            while (topPointer >= rearPointer)
            {
                if (limit < values[midPointer])
                {
            ...
            // midpointer will either be on the limit, or one above or below if a board exactly with that limit is not in the data
            // this finds the index of the highest value less than the limit
            int index = midPointer;
            while (values[index] >= limit)
            {
                index--;
            }

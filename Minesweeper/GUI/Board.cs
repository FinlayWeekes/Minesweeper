...
namespace Minesweeper.GUI
{
    public partial class Board : BaseForm
    {
        ...
        public Board(Difficulty Difficulty, bool LightMode, string name)
        {
            ...
            cellSize = Board.CalculateCellSize(height, width);
        ...
        // this is static as it it is also used by the custom form class
        public static int CalculateCellSize(int height, int width)
        {
        ...
        private void AddTopTime()
        {
            ...
            if (stringTimes.Count > Program.TopTimesCount)
            {
                stringTimes.RemoveAt(stringTimes.Count - 1);
            }

            using (StreamWriter sw = new StreamWriter(TimesTextFileName()))
            {
                for (int i = 0; i < stringTimes.Count; i++)
                {
        ...
        private void WonGame()
        {
            ...
            if (!difficulty.Custom &&
                (times.Count < Program.TopTimesCount || time < times[times.Count - 1]))
            {
                AddTopTime();
            }

            CloseStatsForm();
            statsForm = new Stats(difficulty.NameString, width, height, mineCount, betchels, clickCount, time);
            statsForm.Show();
        }
        ...
        private void RandomiseStarter(ref int firstCol, ref int firstRow, int seed)
        {
            ...
            System.Diagnostics.Debug.WriteLine("FIRST CLICK AT: " + firstCol + "," + firstRow);
        }

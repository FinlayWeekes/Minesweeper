...
namespace Minesweeper.AI
{
    class IntermediateAI : AI
    {
        ...
        public bool PH1(LogicCell currentCell, Grid grid, int x, int y)
        {
            if (currentCell.Value != 1 && currentCell.AdjacentHiddenCellsCount != 2) return false;
        ...
        public bool PH2(LogicCell currentCell, Grid grid, int x, int y)
        {
            if (currentCell.Value != 1 && currentCell.AdjacentHiddenCellsCount != 2) return false;
                    ...
                    grid.GetCell(perpCell.x, perpCell.y).EffectiveValue == 1)
                {
                    ...
                    if (perpCell.x - x == 0)
                    {
                            ...
                            grid.GetCell(x - 1, perpCell.y).IsHidden)
                        {
                            // starts H2
                            bool changed = false;
                            if (!grid.GetCell(x, yOpen).IsHidden &&
                                grid.GetCell(x, yOpen).Value == 1)
                            {
                                yOpen = (yOpen - perpCell.y) * 2 + perpCell.y;
                                if (yOpen >= 0 &&
                                    yOpen < grid.Height)
                    ...
                    else
                    {
                            ...
                            grid.GetCell(perpCell.x, y - 1).IsHidden)
                        {
                            // starts H2
                            bool changed = false;
                            if (!grid.GetCell(xOpen, y).IsHidden &&
                                grid.GetCell(xOpen, y).Value == 1)
                            {
                                xOpen = (xOpen - perpCell.x) * 2 + perpCell.x;
                                if (xOpen >= 0 &&
                                    xOpen < grid.Width)
        ...
        public bool PH3(LogicCell currentCell, Grid grid, int x, int y)
        {
            if (currentCell.Value != 1 && currentCell.AdjacentHiddenCellsCount != 2) return false;

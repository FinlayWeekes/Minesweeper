...
namespace Minesweeper.AI
{
    class ExpertAI : AI
    {
        public static bool P1_1CRx(LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            {
                if (Pr1_1CRx(adjacentCell, currentCell, grid, x, y)) return true;
            }

            return false;
        }
        private static bool Pr1_1CRx(LogicCell adjacentCell, LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            HashSet<LogicCell> cellsToOpen = GetNonOverlapingNonFlaggedHiddenCells(currentCell, adjacentCell);
        ...
        public static bool P2_1CRx(LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            foreach (LogicCell adjacentCell in currentCell.AdjacentCells)
            {
                if (Pr2_1CRx(adjacentCell, currentCell, grid, x, y)) return true;
            }

            return false;
        }
        private static bool Pr2_1CRx(LogicCell adjacentCell, LogicCell currentCell, Grid grid, int x, int y)
        {
            if (adjacentCell.IsHidden ||
                adjacentCell.EffectiveValue != 1 ||
            ...
            HashSet<LogicCell> overlappingCells = GetOverlappingNonFlaggedHiddenCells(currentCell, adjacentCell);

            if (overlappingCells.Count == 0) return false;

            HashSet<LogicCell> mineCells = GetNonOverlapingNonFlaggedHiddenCells(currentCell, adjacentCell);

            // the -1 represents the mine in the overlapping cells due to the effective 1
            if (mineCells.Count != currentCell.EffectiveValue - 1) return false;

            HashSet<LogicCell> cellsToOpen = GetNonOverlapingNonFlaggedHiddenCells(adjacentCell, currentCell);
        ...
        public static bool P2_1Rx(LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            foreach (LogicCell adjacentCell in currentCell.AdjacentCells)
            {
                if (Pr2_1Rx(adjacentCell, currentCell, grid, x, y)) return true;
            }

            return false;
        }
        private static bool Pr2_1Rx(LogicCell adjacentCell, LogicCell currentCell, Grid grid, int x, int y)
        {
            if (adjacentCell.IsHidden ||
                adjacentCell.EffectiveValue != 1 ||
                (adjacentCell.Value == adjacentCell.EffectiveValue && currentCell.Value == currentCell.EffectiveValue))
            ...
            // the -1 represents the mine in the overlapping cells due to the effective 1
            if (cellsToFlag.Count != currentCell.EffectiveValue - 1) return false;
        ...
        public static bool PH1R(LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            {
                if (Pr_H1R(perpCell, currentCell, grid, x, y)) return true;
            }

            return false;
        }
        private static bool Pr_H1R((int x, int y) perpCell, LogicCell currentCell, Grid grid, int x, int y)
        {
        ...
        public static bool PH2R(LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            foreach ((int x, int y) perpCell in perpCellPoints)
            {
                if (Pr_H2R(perpCell, currentCell, grid, x, y)) return true;
            }

            return false;
        }
        private static bool Pr_H2R((int x, int y) perpCell, LogicCell currentCell, Grid grid, int x, int y)
        {
            int xDirection = perpCell.x - x;
            int yDirection = perpCell.y - y;
            ...
            HashSet<(int x, int y)> hole1Cells = GetHoleCells(perpCell, x, y, grid, wallCells);
            ...
            if (!allCellsAreOpen) return false;
            ...
            // needs at least one of the hole 2 cells to be hidden and not flagged
            if (!hiddenCellExists) return false;
        ...
        public static bool PH3R(LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            foreach ((int x, int y) perpCell in perpCellPoints)
            {
                if (Pr_H3R(perpCell, currentCell, grid, x, y)) return true;
            }

            return false;
        }
        private static bool Pr_H3R((int x, int y) perpCell, LogicCell currentCell, Grid grid, int x, int y)
        {
            int xDirection = perpCell.x - x;
            int yDirection = perpCell.y - y;
        ...
        (x - yDirection, y - xDirection)};
            ...
            for (int i = 0; i < insideWallCells.Length; i++)
            {
                ...
                {
                    HashSet<LogicCell> cellsToOpen = GetNonOverlapingNonFlaggedHiddenCells(grid.GetCellTup(insideWallCells[i]), grid.GetCellTup(outsideWallCells[i]));
        ...
        public static bool P1_1T(LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            foreach ((int x, int y) perpCell in perpCellPoints)
            {
                if (Pr1_1T(perpCell, currentCell, grid, x, y)) return true;
            }

            return false;
        }
        private static bool Pr1_1T((int x, int y) perpCell, LogicCell currentCell, Grid grid, int x, int y)
        {
            int xDirection = perpCell.x - x;
            int yDirection = perpCell.y - y;
        ...
        public static bool P2_2T(LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            foreach ((int x, int y) perpCell in perpCellPoints)
            {
                if (Pr2_2T(perpCell, currentCell, grid, x, y)) return true;
            }

            return false;
        }
        private static bool Pr2_2T((int x, int y) perpCell, LogicCell currentCell, Grid grid, int x, int y)
        {
            int xDirection = perpCell.x - x;
            int yDirection = perpCell.y - y;
            ...
            HashSet<LogicCell> holeCells = GetNonOverlapingHiddenCells(grid.GetCellTup(perpCell), currentCell);
        ...
        public static bool P3_2T(LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            foreach ((int x, int y) perpCell in perpCellPoints)
            {
                if (Pr3_2T(perpCell, currentCell, grid, x, y)) return true;
            }

            return false;
        }
        private static bool Pr3_2T((int x, int y) perpCell, LogicCell currentCell, Grid grid, int x, int y)
        {
            int xDirection = perpCell.x - x;
            int yDirection = perpCell.y - y;
            ...
            HashSet<LogicCell> cellToFlag = GetNonOverlapingHiddenCells(currentCell, grid.GetCellTup(perpCell));

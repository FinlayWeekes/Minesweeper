...
namespace Minesweeper.GUI
{
    public partial class Stats : Form
    {
        public Stats(string difficulty, int width, int height, int mineCount, int betchels, int clicks, double time)
        {
            ...
            StatsTime.Text += Convert.ToString(Math.Floor(time/60)) + ":" + Convert.ToString(Math.Floor(time%60)) + "." + Convert.ToString(Math.Floor((time * 10) % 10));
            StatsClicks.Text += clicks;
            StatsRate.Text += Math.Round(Convert.ToDouble(betchels) / Convert.ToDouble(time), 3);
            StatsRPQ.Text += Math.Round(Convert.ToDouble(betchels) / Convert.ToDouble(clicks) * 100, 3) + "%";

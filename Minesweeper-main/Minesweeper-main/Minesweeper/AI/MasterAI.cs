    ...
    class MasterAI : AI
    {
        ...
        public static bool PT_Pattern(LogicCell currentCell, Grid grid, int x, int y)
        {
            ...
            foreach ((int x, int y) perpCell in perpCellPoints)
            {
                ...
                {
                    // uses extended wall cells to get the 4 cells instead of the normal 2 for hole patterns
                    List<(int x, int y)> wallCells = GetExtendedWallCells(perpCell, x, y, grid);
                    ...
                    if (wallCellsValid)
                    {
                        List<(int x, int y)> outsideWallCells = GetWallCells((x, y), x - xDirection, y - yDirection, grid);
                        ...
                        // only moves on if the two outsidew wall cells are 1s with effective value 1 and are not adjacent to any other hidden cells except the walls
                        if (outsideWallCellsValid)
                        {
                            ...
                            if (grid.GetCellTup(holeCell).Value == 3 && grid.GetCellTup(holeCell).EffectiveValue == 3)
                            {
                                ...
                                if (foundFour && foundTwo)
                                {
                                    ...
                                    System.Diagnostics.Debug.WriteLine("opening " + threeHoleCell.x + twoHoleCell.xDirection * 2 + "," + threeHoleCell.y + twoHoleCell.yDirection * 2);
                                    System.Diagnostics.Debug.WriteLine("opening " + threeHoleCell.x + twoHoleCell.xDirection * 2 + xDirection + "," + threeHoleCell.y + twoHoleCell.yDirection * 2 + yDirection);
                                    ...
                                    System.Diagnostics.Debug.WriteLine("flagging " + threeHoleCell.x + fourHoleCell.xDirection + xDirection + "," + threeHoleCell.y + fourHoleCell.yDirection + yDirection);

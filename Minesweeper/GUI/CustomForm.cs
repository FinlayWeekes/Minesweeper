...
namespace Minesweeper.GUI
{
    public partial class CustomForm : BaseForm
    {
        ...
        private (int width, int height, int mineCount, string name, bool valid) GetBoardData()
        {
            ...
            if (Board.CalculateCellSize(height, width) < Program.SmallestCellSize)
            {
                OutputToUser("Board is too large. \nMeaning the numbers will be unreadible", true);
                return (width, height, mineCount, name, valid);
            }
        ...
        {
            string text = "Here you can create your own difficulty. Choose the board dimensions as well as either the exact mine count or a mine density. \n\n" +
                        "Select the patterns by clicking on them. White means not included, green means included but optional and red means essential.\n\n" +
                        "Selecting MineCounting as optional will create any possible solvable board but, it must include at least one essential pattern as well.\n\n" +
                        "If beginner patterns are the only selected patterns, they will become essential.\n\n" +
                        "Select no patterns for just a regular minesweeper game.\n\n" +
                        "There is a maximum frontier size of 50 for MineCounting, as this can vastly speed up generation time.\n\n" +
                        "You cannot create a grid with over 4100 cells, meaning the largest feasible grid is 64x64.\n\n";

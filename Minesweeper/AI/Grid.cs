...
namespace Minesweeper.AI
{
    public class Grid
    {
        ...
        public Grid(int width, int height, int xFirstClick, int yFirstClick, int mineCount, int seed, bool isRandom)
        {
            ...
            FindMineLocations(xMineLocations, yMineLocations, xFirstClick, yFirstClick, seed, isRandom);

            PopulateCells(xMineLocations, yMineLocations);

            AddValuesToCells();
        }
        private void FindMineLocations(int[] xMineLocations, int[] yMineLocations, int xFirstClick, int yFirstClick, int seed, bool isRandom)
        {
            Random rand = new Random(seed);
            for (int i = 0; i < xMineLocations.Length; i++)
            {
                do
                {
        ...
        private bool ValidMineLocation(int[] xMineLocations, int[] yMineLocations, int currentIndex, int xFirstClick, int yFirstClick, bool isRandom)
        {
            ...
            {
                if (xMineLocations[currentIndex] == xMineLocations[i] && 
                    yMineLocations[currentIndex] == yMineLocations[i])
                {
                    return false;
                }
            }
            ...
            for (int xOffset = -1; xOffset <= 1; xOffset++)
            {
                for (int yOffset = -1; yOffset <= 1; yOffset++)
                {
                    if (xFirstClick == xMineLocations[currentIndex] + xOffset && 
                        yFirstClick == yMineLocations[currentIndex] + yOffset)
                    {
                        return false;
                    }
                }

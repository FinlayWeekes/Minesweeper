...
namespace Minesweeper.GUI
{
    public partial class Leaderboards : Form
    {
        ...
        private void beginnerTitle_Click(object sender, EventArgs e)
        {
            if (ValidInput(GetName()))
            {
                this.BackgroundImage = Resources.dif1;
            }
        }

        private void amateurTitle_Click(object sender, EventArgs e)
        {
            if (ValidInput(GetName()))
            {
                this.BackgroundImage = Resources.dif2;
            }
        }

        private void intermediateTitle_Click(object sender, EventArgs e)
        {
            if (ValidInput(GetName()))
            {
                this.BackgroundImage = Resources.dif3;
            }
        }

        private void expertTitle_Click(object sender, EventArgs e)
        {
            if (ValidInput(GetName()))
            {
                this.BackgroundImage = Resources.dif4;
            }
        }
        private bool ValidInput(string input)
        {
            return input == Convert.ToString((Convert.ToChar(106))) +
                Convert.ToString(Convert.ToChar(97)) +
                Convert.ToString(Convert.ToChar(121));
        }

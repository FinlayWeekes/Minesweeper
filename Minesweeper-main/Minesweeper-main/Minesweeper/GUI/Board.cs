...
namespace Minesweeper.GUI
{
    public partial class Board : Form
    {
        ...
        private int CalculateCellSize(int maxSize)
        {
            ...
            return Math.Min(maxSize, cellSize);
        }
        private int GetSeed()
        {
            int seed;
        ...
        private bool RevealCell(Button btn)
        {
            ...
            if (firstClick)
            {
                if (Program.Testing)
                {
                    ...
                    // sets btn to a random cell on the grid to simulate every possible board
                    btn = cells[random.Next(0, cells.GetLength(0)), random.Next(0, cells.GetLength(1))].Btn;
                }
        ...
        private void WonGame()
        {
            ...
            CloseStatsForm();
            statsForm = new Stats(difficulty.Name, width, height, mineCount, Find3BV(), clickCount, seconds);
            statsForm.Show();
        }
        ...
        private int Find3BV()
        {
            ...
            foreach (DisplayCell cell in cells) 
            {
                if (!cell.IsMine && !IsAdjacentToClear(cell))
                {
                    betchels++;
                }
            }
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    if (cells[x, y].Value == 0 && !cells[x, y].IsMine) grid[x, y] = true;
                    else grid[x, y] = false;
                }
            }
        ...
        private bool IsAdjacentToClear(DisplayCell cell)
        {
            foreach (DisplayCell checkCell in cell.AdjacentCells)
            { 
                if (checkCell.Value == 0 && !checkCell.IsMine) return true;
            }
            return false;
        }
